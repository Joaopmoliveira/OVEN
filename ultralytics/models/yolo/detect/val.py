...
class DetectionValidator(BaseValidator):
    ...
    def postprocess(self, preds: torch.Tensor) -> list[dict[str, torch.Tensor]]:
        ...
        """
        if isinstance(preds, (list, tuple)):  # TODO: This is a fix due to local stupid torch install version, it should not be here
            preds = preds[0].cpu()  # select only inference output
    ...
    def _process_batch(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> dict[str, np.ndarray]:
        ...
        iou = box_iou(batch["bboxes"].cpu(), preds["bboxes"].cpu())
        return {"tp": self.match_predictions(preds["cls"].cpu(), batch["cls"].cpu(), iou).cpu().numpy()}
