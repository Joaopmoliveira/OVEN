...
class Instances:
    ...
    def scale(self, scale_w: float, scale_h: float, bbox_only: bool = False):
        ...
        """
        before_areas = self._bboxes.areas()
        self._bboxes.mul(scale=(scale_w, scale_h, scale_w, scale_h))
        after_areas = self._bboxes.areas()
        if bbox_only:
            return
        ratio = after_areas / before_areas
    ...
    def __getitem__(self, index: int | np.ndarray | slice) -> Instances:
        ...
        extras = self.extras[index] if self.extras is not None else None
    ...
    def flipud(self, h: int) -> None:
        ...
        if self.extras is not None:
            ...
            b = self.extras[:,11:12]
            l = self.extras[:,12:13]
            self.extras[:,9:10] = -b+1
            self.extras[:,10:11] = -r+1
            self.extras[:,11:12] = -t+1
            self.extras[:,12:13] = -l+1
    ...
    def fliplr(self, w: int) -> None:
        ...
        if self.extras is not None: #we must switch the left 
            ...
            b = self.extras[:,11:12]
            l = self.extras[:,12:13]
            self.extras[:,9:10] = -t+1
            self.extras[:,10:11] = -l+1
            self.extras[:,11:12] = -b+1
            self.extras[:,12:13] = -r+1
    ...
    def clip(self, w: int, h: int) -> None:
        ...
        #if self.extras is not None:
        #    self.extras += 0 ## we do nothing because we actually don't know how the roof is oriented in the image, hence we must either ignore it 
            # and force the model to predict it or we must let it go
    ...
    def remove_zero_area_boxes(self) -> np.ndarray:
        ...
        if not all(good):
            ...
            if self.extras is not None:
                self.extras = self.extras[good]
        return good

    def update(self, bboxes: np.ndarray, segments: np.ndarray = None, keypoints: np.ndarray = None, extras: np.ndarray = None):
        """
    ...
    def concatenate(cls, instances_list: list[Instances], axis=0) -> Instances:
        ...
        use_extras = instances_list[0].extras is not None
        ...
        cat_extras = np.concatenate([b.extras for b in instances_list], axis=axis) if use_extras else None
        return cls(cat_boxes, cat_segments, cat_keypoints, cat_extras, bbox_format, normalized)
