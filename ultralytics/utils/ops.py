...
def regularize_roofboxes(rboxes):
    """
    Regularize rooftop angles to range [0, pi/2].

    Args:
        rboxes (torch.Tensor): Input rotated boxes with shape (N, 4+13) in xywh a_hori,a_top,a_right,a_bottom,a_left ... format.

    Returns:
        (torch.Tensor): Regularized rotated boxes.
    """
    x, y, w, h, a_horizontal,a_top,a_right,a_bottom,a_left,inc_top,inc_right,inc_bottom,inc_left,ori_top,ori_right,ori_bottom,ori_left = rboxes.unbind(dim=-1)

    inc_top = inc_top % (math.pi / 2)
    inc_right = inc_right % (math.pi / 2)
    inc_bottom = inc_bottom % (math.pi / 2)
    inc_left = inc_left % (math.pi / 2)

    ori_top = ori_top % (math.pi / 2)
    ori_right = ori_right % (math.pi / 2)
    ori_bottom = ori_bottom % (math.pi / 2)
    ori_left = ori_left % (math.pi / 2)
