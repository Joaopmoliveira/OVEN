...
class Results(SimpleClass, DataExportMixin):
    ...
    Attributes:
        ...
        roof (RoofTop | None): Roof Topologies.
    ...
    def __init__(
        ...
        self.roof = RoofTop(roof, self.orig_shape) if roof is not None else None
        self.speed = speed if speed is not None else {"preprocess": None, "inference": None, "postprocess": None}
        self.names = names
        self.path = path
        self.save_dir = None
        self._keys = "boxes", "masks", "probs", "keypoints", "obb", "roof"
    ...
    def update(
        ...
        if roof is not None:
            self.roof = RoofTop(roof,self.orig_shape)
...
class RoofTop(BaseTensor):
    """
    A class for storing and manipulating rooftop topologies.

    This class encapsulates functionality for handling rooftop data, including coordinate manipulation,
    normalization, and confidence values. It supports rooftop detection results with optional visibility
    information.

    Attributes:
        data (torch.Tensor): The raw tensor containing keypoint data.
        orig_shape (tuple[int, int]): The original image dimensions (height, width).
        has_visible (bool): Indicates whether visibility information is available for rooftop.
        xy (torch.Tensor): Keypoint coordinates in [x, y] format.
        xyn (torch.Tensor): Normalized keypoint coordinates in [x, y] format, relative to orig_shape.
        conf (torch.Tensor): Confidence values for each keypoint, if available.
    ...
    def __init__(self, rooftops: torch.Tensor | np.ndarray, orig_shape: tuple[int, int]) -> None:
        """
        Initialize the Keypoints object with detection keypoints and original image dimensions.
        ...
        if rooftops.ndim == 2:
            data = rooftops[None, :]
        super().__init__(rooftops, orig_shape)
        self.has_visible = self.data.shape[-1] == 3
