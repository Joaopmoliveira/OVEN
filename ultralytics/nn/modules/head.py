...
class OBB(Detect):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        ...
        #print(f"shape: 0:[[{x[0].shape}]] 1:[[{x[1].shape}]] 2:[[{x[2].shape}]]")
...
class RoofDetect(Detect):
    ...
    Args:
        ...
        ne (int): number of extra regression outputs per anchor (default 5:)
        ch (tuple): channels for backbone feature maps (e.g. (256,512,1024)).
    """

    def __init__(self, nc: int = 1, ne: int = 5, ch: tuple = ()):
        super().__init__(nc = nc, ch = ch)           # initialize parent Detect (anchors, nl, stride, etc.)
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        ...
        #extras = extras.sigmoid() # all predictions are in between zero and one
        if not self.training:
            self.extras = extras
        x = Detect.forward(self, x)
        if self.training:
