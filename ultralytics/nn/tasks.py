...
class RoofModel(DetectionModel):
    ...
    Examples:
        ...
        >>> model = RoofModel("yolo11n-roof.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="yolo11n-roof.yaml", ch=3,nc=1, verbose=True):
        """
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, RoofDetect, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, RoofDetect}:
                m.legacy = legacy
...
def guess_model_task(model):
    ...
    Returns:
        (str): Task of the model ('detect', 'segment', 'classify', 'pose', 'obb','roof').
    """
    ...
    if isinstance(model, (str, Path)):
        ...
        if "-seg" in model.stem or "segment" in model.parts:
            ...
            return "obb"
        elif "-roof" in model.stem or "roof" in model.parts:
            return "roof"
    ...
    LOGGER.warning(
        ...
        "Explicitly define task for your model, i.e. 'task=detect', 'segment', 'classify','pose','obb' or 'roof'."
    )
